...
sub prt_text_issue {
	...
	$multi_quits_last = undef;
	$dest->{hide} = 1;
	if ($dest->{level} & MSGLEVEL_QUITS) {
	    $multi_quits_last = $stripd;
	}
    }
    elsif (ref $dest && $dest->{level} & MSGLEVEL_QUITS
	       && defined $multi_quits_last && $multi_quits_last eq $stripd) {
	$dest->{hide} = 1;
    }
    else {
	$multi_quits_last = undef;
